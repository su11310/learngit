...
{
    UINT i,j,k,chalen;
    UINT8 chale[8],key1[8];
    UINT8 en_out_put[8];
    k=0xf5;
    memset(chale,0,0x08);
//    memset(key1,0x01,0x08);

    chalen=8;
    i= card_getchal(chale,chalen);

    des(chale, KEY_CAR_PASS, en_out_put, DES_ENCRYPT);
 
    j=external_authentication(k, en_out_put, 0x08);
    return j;
}