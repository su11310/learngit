...
//}
void  OnceRecordEEP_SP()                                                        //错误的存储区域 ss190128
{
    UINT8 OnceID;
    if(1 != o24lcxxx_read_bytes_ex(GAS_RECORD_SREIAL_FLAG_SP,&OnceID,1))          // 读取存储序列号标志
    {
    ...
    if( OnceID != 0x57)
    {
        ...
        o24lcxxx_write_bytes_ex(GAS_RECORD_SREIAL_FLAG_SP,&OnceID,1);              // 写入存储标志
        o24lcxxx_write_bytes_ex(GAS_RECORD_SREIAL_SP,record_id.compa_uc,4);        // 写入序列号为零
    }
//    OnceID = 0x55; 
//    o24lcxxx_write_bytes_ex(GAS_RECORD_SREIAL_FLAG,&OnceID,1);
}
UINT32 GasRecordMAX_SP()                                                        //获取最大序列号 错误的
{
    Compa_u32 record_id;
    o24lcxxx_read_bytes_ex(GAS_RECORD_SREIAL_SP,record_id.compa_uc,4);                       // 读取存储序列号
    return record_id.compa_u32;                                                       /// 获取最大键盘板加气记录序列号
}
BOOL WriteRecordSerial_SP(UINT32 id)                                                  //  写入序号 
{
    Compa_u32 record_id;
    record_id.compa_u32 = id;
    
    if(4 != o24lcxxx_write_bytes_ex(GAS_RECORD_SREIAL_SP,record_id.compa_uc,4))                           // 读取存储序列号
    {
        if(4 != o24lcxxx_write_bytes_ex(GAS_RECORD_SREIAL_SP,record_id.compa_uc,4))                       // 读取存储序列号
        {
            return FALSE;
...
BOOL WriteRecordDate_SP(UINT32 id,GAS_log date)                                       //  将加气异常的记录存储键盘板   30组数据
{
    UINT8 len;
    shift_type write_date;
    write_date.gas_record_query = date;
    len = sizeof(write_date.compa_uint8);
    if(len != o24lcxxx_write_bytes_ex((GAS_RECORD_ADD_SP+(((id-1)%100)*RECORD_LEN_SP)), write_date.compa_uint8,len))             // 读取键盘板加气记录信息
    {
        if(len != o24lcxxx_write_bytes_ex((GAS_RECORD_ADD_SP+(((id-1)%100)*RECORD_LEN_SP)), write_date.compa_uint8,len))             // 读取键盘板加气记录信息   30 -> 100  ss190308
        {
...
//    
//}
void WiteRecorInfo_SP(GAS_log date)                                                 // 写入 加气记录信息
{
    UINT32 id;
    id = GasRecordMAX_SP();    // 最大记录序列号
    id = id+1;                                                                  //写入 ID 序列号增加
    if(!WriteRecordSerial_SP(id))                                                  // 写入 ID 序号
    {
        SOEACTION1(EEPROM_WRITE_KEY_ERR,eCurRunState);                                      // 获取 EEP 数据错误  
        return;
    }
    if(!WriteRecordDate_SP( id , date ))                                            // 写入 记录信息到指定地址
    {
        SOEACTION1(EEPROM_WRITE_KEY_ERR,eCurRunState);                                      // 获取 EEP 数据错误  
        return;
    }
...
{
    WiteRecorInfo(date) ;                                                       //正常存储加气情况的记录
    
//      if((date.gas_stop_way != MONEY_ALL)&&(date.gas_stop_way != PRESS_STOP)&& \
//            (date.gas_stop_way != BACKSTAGE_STOP)&&(date.gas_stop_way != VELOCIIY_LOW)) // 不等于正常停止加气（金额加满、停止键、后台停止、流速低）    ss190215
//    {
//        WiteRecorInfo_SP(date) ;                                                    // 存储加气异常情况的记录
//        return;
//    }
    if(GAS_START_SEND_SUCC != (GAS_START_SEND_SUCC &date.gas_process))              //加气开始指令发送失败
    {
        WiteRecorInfo_SP(date) ;                                                    // 存储加气异常情况的记录
        return;
    }
    
    if(GAS_PRECOOL_SUCC != date.gas_precool)                                          //预冷失败             ss190215
    {
        WiteRecorInfo_SP(date) ;                                                    // 存储加气异常情况的记录
        return;
    }
    
//    if((shift_read.gas_record_query.grey_lock_inif.grey_status & 0xF0)== GAS_START_MORMAL)     //  定量、定额不需要发送停止指令
//    {
//        if( GAS_STOP_SEND_SUCC != (date.gas_process & GAS_STOP_SEND_SUCC) )           // 发送停止指令发送失败
//        {
//            WiteRecorInfo_SP(date) ;                                                    // 存储加气异常情况的记录
//            return;
//        }   
//    }
    if(((WKJQ==1)&&(_CARD_TYPE < 7))||((WKJQ==0)&&(_CARD_TYPE == 2)||(_CARD_TYPE == 3)))     //如果无卡加气，不判断灰锁状态
    {
        if(GREY_LUCK_SUCC != (date.grey_lock_inif.grey_status & 0x03))    // 加气记录   加气前灰锁状态失败
        {
            WiteRecorInfo_SP(date) ;                                                    // 存储加气异常情况的记录
            return;
        }

        if(GREY_GAS_LUCK_SUCC != (date.grey_lock_inif.grey_status & 0x0C))         // 加气记录   加气过程中再次读取灰锁状态
        {
            WiteRecorInfo_SP(date) ;                                                    // 存储加气异常情况的记录
            return;
        } 

        if((date.grey_unlock_inif.grey_status & 0xF0) != GREY_UNLUCK_SUCC)          //解灰状态失败
        {
            WiteRecorInfo_SP(date) ;                                                    // 存储加气异常情况的记录
            return;
        }
    ...
    if( GAS_ACCOUNTS_SEND_SUCC != (date.gas_process & GAS_ACCOUNTS_SEND_SUCC))         // 通知主板保存加气信息
    {
        WiteRecorInfo_SP(date) ;                                                    // 存储加气异常情况的记录
        return;
    }
...
BOOL ReadRecordInfo_SP( UINT32 id)                                                  //  读取键盘板加气记录信息
{
//    shift_type shift_read;
    UINT8 len;
    len = sizeof(shift_read.compa_uint8);
    if(len != o24lcxxx_read_bytes_ex((GAS_RECORD_ADD_SP+(((id-1)%100)*RECORD_LEN)), shift_read.compa_uint8,len))             // 读取键盘板加气记录信息
    {
        if(len != o24lcxxx_read_bytes_ex((GAS_RECORD_ADD_SP+(((id-1)%100)*RECORD_LEN)), shift_read.compa_uint8,len))             // 读取键盘板加气记录信息
        {
...
void DispRecordInfo_SP(UINT32 id)                                                                   //显示当前 ID 加气流水号信息
{
//    UINT8 card_id[8];
     UINT8 buffer[10];
...
{
    UINT32 MAX_id,i; 
    MAX_id = GasRecordMAX_SP();                                               // 最大记录序列号
     if(MAX_id > 30)
        i = MAX_id - 30;
//     MAX_id = (MAX_id % 30)+1;
...
void GasRecordQueryIndex_SP(void)                       // 索引查询键盘板加气记录信息
{
    UINT32 MAX_id,id; 
    UINT32 liushuinum = 0;
    UINT8 len, vkey = 0;
